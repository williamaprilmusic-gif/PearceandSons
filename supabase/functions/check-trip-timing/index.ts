...
//      never having reached IN_TRANSIT — latestartnotified flag.
    ...
    for (const t of trips || []) {
      ...
      if (t.status === "IN_TRANSIT" && !t.stuckintransitnotified && t.intransitat) {
        ...
        if (msStuck >= THREE_HOURS_MS) {
          ...
          if (t.driverid) {
            notifRows.push({ title: "TRIP STUCK IN TRANSIT", type: "TRIP_STUCK_IN_TRANSIT", forroles: ["DRIVER"], userid: t.driverid, message: `⚠ Trip ${t.id} still shows as in progress — please mark it complete if you've finished, or contact dispatch.`, tripid: t.id, timestamp: nowMs, isread: false });
          }
          await supabase.from("notifications").insert(notifRows);
